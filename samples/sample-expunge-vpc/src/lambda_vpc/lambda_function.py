...
def create_ec2_client(region_name, **kwargs):
    if "profile" in kwargs:
        logger.info(
            "Creating Boto3 EC2 Client using profile: {}".format(kwargs["profile"])
        )
...
def delete_subnets(client, vpc_id):
    ...
    subnet = client.describe_subnets(Filters=[{"Name": "vpc-id", "Values": [vpc_id]}])
...
def delete_internet_gateway(client, vpc_id):
    ...
    for gw in igw["InternetGateways"]:
        ...
        client.detach_internet_gateway(
            InternetGatewayId=gw["InternetGatewayId"], VpcId=vpc_id
        )
        logger.info(f"Deleting internet gateway: {gw['InternetGatewayId']}")
        client.delete_internet_gateway(InternetGatewayId=gw["InternetGatewayId"])
