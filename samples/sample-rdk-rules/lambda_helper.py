...
def replace_rule_name_and_load(
    file: str, rule_name: str, rule_name_stripped: str
) -> dict:
    try:
...
def add_lambda_to_template_by_rule(
    template: dict, config_rule_dir: str, rule_name: str, s3_asset_key: str
) -> dict:
    ...
    lambda_role = replace_rule_name_and_load(
        Path(templates_root).joinpath("lambda-role.json"), rule_name, rule_name_stripped
    )
    template["Resources"][f"{rule_name_stripped}LambdaRole"] = lambda_role

    lambda_function = replace_rule_name_and_load(
        Path(templates_root).joinpath("lambda-function.json"),
        rule_name,
        rule_name_stripped,
    )
    ...
    lambda_permission = replace_rule_name_and_load(
        Path(templates_root).joinpath("lambda-permission.json"),
        rule_name,
        rule_name_stripped,
    )
...
def main(shared_modules_bucket: str):
    ...
    for config_rule_dir in config_rules_dirs:
        ...
        file_asset_path = shutil.make_archive(
            Path(config_rule_dir).joinpath(config_rule_dir.name), "zip", config_rule_dir
        )
        ...
        template = add_lambda_to_template_by_rule(
            template, config_rule_dir, rule_name, s3_asset_key
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--asset-folder",
        required=False,
        help="[optional] Asset folder in the ADF bucket",
    )
    parser.add_argument(
        "--template-name", required=True, help="Name for the generated template name"
    )
    parser.add_argument(
        "-r",
        "--region",
